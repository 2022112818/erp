...
const form = reactive({ id: "", name: "", address: "", contactPerson: "", phone: "" })

function getList() {
  request.get('/customer/list').then(res => { tableData.value = res.data })
}
getList()
...
function save() {
  if (form.id) {
    request.put('/customer/update', form).then(() => { dialogVisible.value = false; getList() })
  } else {
    request.post('/customer/create', form).then(() => { dialogVisible.value = false; getList() })
  }
}

function remove(id) {
  request.delete(`/{'customer/delete/'}${id}`).then(() => { getList() })
}
</script>
